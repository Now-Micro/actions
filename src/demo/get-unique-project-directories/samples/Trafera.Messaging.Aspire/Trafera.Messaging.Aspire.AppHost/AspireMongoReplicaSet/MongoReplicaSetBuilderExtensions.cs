    ...
    )
    {
        ...
        builder.Eventing.Subscribe<ConnectionStringAvailableEvent>(
            ...
            async (@event, ct) =>
            {
                ...
                options.LoggingSettings = new LoggingSettings(
        ...
        foreach (
            var annotation in mongoDbServerResource
        ...
        )
        {
            ...
            {
                var mongoDbServerHealthCheck = options.Registrations.FirstOrDefault(x =>
                    x.Name == annotation.Key
                );
